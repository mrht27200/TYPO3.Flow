...
class ProxyClassBuilder {
	 ...
	 * @return string
	 */
	protected function buildSerializeRelatedEntitiesCode(
		...
		foreach ($objectConfiguration->getProperties() as $propertyName => $propertyConfiguration) {
			...
			switch ($propertyConfiguration->getType()) {
				...
				case ConfigurationProperty::PROPERTY_TYPES_STRAIGHTVALUE:
					...
					$commands = array('$this->' . $propertyName . ' =' . $preparedSetterArgument . ';');
				break;
	...
	protected function buildPropertyInjectionCodeByConfiguration(Configuration $objectConfiguration, $propertyName, Configuration $propertyConfiguration) {
		...
		return $this->buildLazyPropertyInjectionCode($propertyConfiguration->getObjectName(), $propertyClassName, $propertyName, $preparedSetterArgument);
	}
	 ...
	 *
	 * @param string $propertyObjectName Object name of the property object to inject
	...
	protected function buildLazyPropertyInjectionCode($propertyObjectName, $propertyClassName, $propertyName, $preparedSetterArgument) {
		$propertyReferenceVariable = '$propertyVariable' . ucfirst($propertyName);
		$commands[] = 'if (\TYPO3\Flow\Core\Bootstrap::$staticObjectManager->hasInstance(\'' . $propertyObjectName . '\')) {';
		$commands[] = '	$this->' . $propertyName . ' = \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->get(\'' . $propertyObjectName . '\');';
		$commands[] = '} else {';
		$commands[] = '	' . $propertyReferenceVariable . ' = &$this->' . $propertyName . ';';
		$commands[] = '	$this->' . $propertyName . ' = new \TYPO3\Flow\Object\DependencyInjection\DependencyProxy(';
		$commands[] = '		function($methodName = NULL, $arguments = array()) use(&' . $propertyReferenceVariable . ') {';
		$commands[] = '			' . $propertyReferenceVariable . ' = ' . $preparedSetterArgument . ';';
		$commands[] = '			return ($methodName === NULL) ? NULL : call_user_func_array(array(' . $propertyReferenceVariable . ', $methodName), $arguments);';
		$commands[] = '		}, \'' . $propertyClassName . '\'';
		$commands[] = '	);';
		$commands[] = '}';
		return $commands;
	}
	 ...
	 * @return string
	 */
	protected function buildLifecycleInitializationCode(
		Configuration $objectConfiguration, $cause) {
